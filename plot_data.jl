...
# ╔═╡ 3dccb4f0-6c2d-11f1-0890-6b8640c169ee
using DelimitedFiles, CSV, DataFrames

# ╔═╡ 23649522-78db-43fe-9982-39b1fa18bbf9
using Plots

# ╔═╡ 32244e08-8994-4756-a147-3f5c7fd6e249
### Trajectories over OSM roads/hedges/tree-lines — Pluto + PlotlyJS.jl ###
#
# No Mapbox token needed. Instead of a satellite/street raster basemap, this
# queries OpenStreetMap's Overpass API for just three feature types and draws
# them as line layers on a blank map:
#   - roads        (highway=*)
#   - hedges       (barrier=hedge)
#   - tree lines   (natural=tree_row)
#
# Packages needed: PlotlyJS, HTTP, JSON3, Statistics
#   using Pkg; Pkg.add(["PlotlyJS", "HTTP", "JSON3"])
...
# ╔═╡ f8900e8d-4c6d-4779-a33b-013a91d8fb19
...
# ╔═╡ 2dfeba7a-bbd7-4536-bc80-586f318190c9
begin
...
# ╔═╡ 79ed759e-eef2-4a00-9f57-db99b252d595
begin
...
# ╔═╡ 7eb725f3-402c-4896-b3b2-9f5c3df655f2
all_trajs

# ╔═╡ 9a4d4c43-e441-4e52-9787-1e86f0ed3bd5
md"""
# Trajectories over OSM roads, hedges & tree lines
No basemap imagery — only the three vector layers pulled from OpenStreetMap.
"""

# --- Your trajectory data (replace with your real trajectories) -------

# ╔═╡ a6e4d049-ebbf-44b0-aaf0-debe03fc6efc
begin
	trajectories = [
		...
		# ... up to ~400 (lats, lons) tuples
	]
end

# --- Bounding box around all trajectories, with padding ---------------

# ╔═╡ 0eaa53fd-d3a4-4d7c-8eef-e914a5f15332


# ╔═╡ d963cb7f-b599-41b1-ba3d-25a187bd6e6d
...
# ╔═╡ 0f061eaa-6e43-4f1a-a577-e3fa534893cb
bbox = trajectories_bbox(trajectories)

# --- Query the Overpass API for roads, hedges, and tree lines ---------

# ╔═╡ 7b340da3-150f-4604-8080-51cbc1cbf8c3
...
# ╔═╡ 1be30dfd-215f-4f75-a198-701e7986c38c
osm_data = fetch_osm_features(bbox)

# --- Sort the returned ways into roads / hedges / tree lines -----------

# ╔═╡ a149ea23-193d-437d-a510-5c37fb119809
...
# ╔═╡ 0d4cf470-f5ba-4b98-90c5-d7d9cdc1378e
roads, hedges, treelines = classify_features(osm_data)

# --- Merge a list of line-geometries into one trace (missing = break) --

# ╔═╡ 11c03feb-e8fc-457a-8edf-9070fb0f229e
...
# --- Build one trace per feature type ----------------------------------

# ╔═╡ d4000c98-7ab5-4f27-b96e-6fbf3287bfd3
begin
...
# --- Trajectory traces, grouped in batches for color variety + speed ---

# ╔═╡ 2519456c-34ca-4fd4-a272-368a19921e64
begin
	const PALETTE = [
...
# ╔═╡ 7d9b9495-08aa-49f9-8bda-1dc018c7e83c
...
# ╔═╡ 48f91196-4b5f-4376-afa6-7b1cbf99d864
...
# ╔═╡ 312e9d6d-f32d-4d64-a1b2-024e6cea3e8d
trajectory_traces = build_trajectory_traces(trajectories; group_size = 20)

# --- Layout: blank background, no Mapbox token required ----------------

# ╔═╡ fc3df3e0-af7a-4aeb-b2e3-de73b26e80e6
layout = Layout(
	mapbox = attr(
		style = "white-bg",     # blank canvas — only our drawn layers show
		center = attr(lat = mean([bbox.south, bbox.north]),
...
# --- Render: feature layers first, trajectories drawn on top ------------

# ╔═╡ 9175ebbf-a94d-45ea-ae5b-3dc136c0228c
plot([road_trace, hedge_trace, tree_trace, trajectory_traces...], layout)
...
manifest_format = "2.0"
project_hash = "b8e8ec2486c122eb655775810c93f36de0e6ff6a"
